...
def response(data, inxight):
    """
    Initializes an OpenAI gpt-3.5 model with generated prompt 
    """
    
    prompt = f"You are an expert and medical conditions. Return all the values in {data} which are the exact same as {inxight} as a comma seperated list"
...
#     stream = client.chat.completions.create(
#         model="gpt-3.5-turbo",
#         messages=[{"role": "user", "content": f"You are an expert and medical conditions. Return 'yes' if these two terms are synonyms, 'no' otherwise: {cortellis}, {inxight}"}],
#         stream=True,
#     )
#
#     response_content = []
...
#
# @lru_cache(None)
# def response_cached(key, target_string):
#     return response(key, target_string).lower() == 'yes'
...
#         key_lower = key.lower()
#
#         # Check if the current key matches the target string or contains it as a word
#         if key_lower == target_string_lower or target_string_lower in key_lower.split():
#             matches.add(tuple(new_path))
#
#         # Check if the response function returns 'yes' for the current key and target string
#         if response_cached(key, target_string):
#             matches.add(tuple(new_path))
#
#         # If the current value is a dictionary, recursively search
#         if isinstance(value, dict):
#             matches.update(find_all_paths_to_node(value, target_string, new_path))
#
#         print(matches)
#
#     return matches
...
def main():
    with open("data/json/output_pair_1.json", 'r') as file:
        current = json.load(file)
        print(response(current, "Prostatitis"))
